...
async function loadJid() {
  try {
    const current = await invoke<string | null>("cmd_get_my_jid");
...
async function saveJid() {
  ...
  saving.value = "jid";
  try {
    await invoke("cmd_set_jid", { jid: jid.value.trim() });
    savedJid.value = true;
...
async function savePassword() {
  ...
  try {
    await invoke("cmd_set_password", { password: password.value });
    password.value = "";
  ...
  <div class="page">
    ...
    <div class="group">
      ...
      <div class="row">
        ...
        />
        <button @click="saveJid" :disabled="saving === 'jid' || !jid.trim()">
          {{ savedJid ? "Saved ✓" : "Save" }}
        </button>
      </div>
    </div>
    ...
    <div class="group">
      ...
      <div class="row">
        ...
        />
        <button @click="savePassword" :disabled="saving === 'password' || !password">
          {{ savedPassword ? "Saved ✓" : "Save" }}
        </button>
      </div>
      <p class="hint">Stored in the system keyring. Leave blank to keep current value.</p>
    </div>
  </div>
</template>
...
@media (prefers-color-scheme: dark) {
  .row input { background: #3f3f46; border-color: #52525b; }
}
...
.row button:not(:disabled):hover { background: #1d4ed8; }
